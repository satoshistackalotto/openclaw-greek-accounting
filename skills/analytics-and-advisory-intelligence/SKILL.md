...
- **Proactive, Not Reactive**: The system already handles reactive work. This skill looks ahead â€” identifying problems before they become crises and opportunities before they are missed
...
Compliance_Risk_Score:
  ...
  score_bands:
    1-3: "Low risk â€” all obligations current, no gaps"
    4-6: "Medium risk â€” minor gaps or historical delays"
    7-8: "High risk â€” active gaps or recent penalties"
    9-10: "Critical â€” immediate attention required"
...
Financial_Trends:
  metrics_tracked:
    gross_margin:
      ...
      finding_template: "{Client} gross margin has fallen from {X}% to {Y}% â€” {delta}pp decline over {N} months. Primary driver appears to be {top expense category change}."
...
Anomaly_Detection:
  methods:
    ...
    vat_rate_anomaly:
      ...
      action: "Flag for accountant review â€” never auto-correct"
...
Cashflow_Forecast:
  ...
  confidence_degradation:
    ...
    3_months: "LOW confidence â€” directional only"
```
...
Portfolio_Intelligence:
  sector_benchmarks:
    ...
    refreshed: "Monthly â€” after all monthly statements are generated"
  ...
  workload_concentration:
    description: "Risk-weighted workload per accountant â€” alerts if one person carries disproportionate risk"

  common_issues:
    description: "Issues appearing across multiple clients simultaneously"
    examples:
      - "6 retail clients showing declining margins â€” possible sector-wide trend"
...
```
MORNING ADVISORY â€” 19/02/2026
Generated: 19/02/2026 05:47 Athens time

WATCH LIST â€” 3 clients warrant attention today

1. GAMMA CONSTRUCTIONS AE (EL555444333) â€” RISK: HIGH (8/10)
...
2. DELTA SERVICES EPE (EL777888999) â€” RISK: MEDIUM (6/10)
...
3. EPSILON RETAIL OE (EL222333444) â€” RISK: MEDIUM (5/10)
...
Upstream_Skills_Read:
  greek-financial-statements:   "P&L, balance sheet, VAT summary â€” primary financial data"
  ...
  cli-deadline-monitor:         "Upcoming deadlines â€” feeds cash flow forecast"
  greek-compliance-aade:        "Filing history, penalty records"
  client-communication-engine:  "Correspondence patterns â€” identifies chronic document requesters"
...
Memory_Integration:
  ...
  useful_patterns_to_detect:
    - "Anomaly types consistently dismissed by accountants â€” thresholds may be too sensitive"
    - "Risk score bands that consistently predict actual compliance problems â€” calibration quality"
    - "Clients where forecasts are consistently wrong in one direction â€” systematic bias"
...
Error_Responses:
  ...
  insufficient_history:
    output: "{Client} has only {N} periods of data â€” trend analysis requires at least 3. Showing available data with LOW CONFIDENCE marker."
    action: "Produce what is possible, clearly labelled. Do not refuse to show anything."

  sector_too_small:
    output: "Sector benchmark for {sector} cannot be computed â€” only {N} clients (minimum 3 required). Showing own-history analysis only."
...
- Zero false positives that cause unnecessary client alarm â€” confidence ratings are accurate
- All findings in plain English that an accounting assistant can act on immediately
- Read-only confirmed â€” no writes to any operational directory, ever

Remember: This skill is the difference between an accounting system that processes the past and one that helps the firm prepare for the future. The value is not in the data it holds â€” the value is in what it notices that humans would have missed.
